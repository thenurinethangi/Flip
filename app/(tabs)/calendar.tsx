import { default as AddSubtaskModal, default as AddTaskModal } from '@/components/AddSubtaskModal';
...
export default function Calendar() {
  ...
  const surfaceBg = isDark ? '#000000' : '#F5F6F8';
  ...
  return (
    <>
      <SafeAreaView className={`flex-1 pb-0 mb-0 ${isDark ? 'bg-[#000000]' : 'bg-[#F5F6F8]'}`}>
        ...
        <View className='mt-[17px] flex-row justify-between items-center px-4'>
          <View className='flex-row items-center gap-x-4'>
            ...
            <Text className='text-[20.5px] font-semibold' style={{ color: textPrimary }}>{monthStr}</Text>
          </View>
          <View className=''>
        ...
        <CalendarProvider
          date={todayStr}
          style={{ backgroundColor: surfaceBg, flex: 0 }}
          onDateChanged={(d) => {
            setChoooseDate(d);
        ...
        >
          <ExpandableCalendar
            ...
            theme={{
              ...
              dayTextColor: isDark ? '#E5E7EB' : '#3B3B3B',
              textSectionTitleColor: isDark ? '#9CA3AF' : '#7A818E',
              textDayHeaderFontSize: 12.1,
              textDayHeaderFontWeight: '500',
              todayBackgroundColor: surfaceBg,
        ...
        {/* tasks container */}
        <View className='-translate-y-2 pb-0 mb-0' style={{ flex: 1, paddingHorizontal: 4, paddingTop: 0, paddingBottom: 0, marginBottom: 0, backgroundColor: surfaceBg }}>
          <ScrollView
            style={{ flex: 1 }}
          ...
          >
            ...
            {incompleteTasks.length > 0
              ? <View className='shadow-lg shadow-black/0.05 px-2 pb-2' style={{ backgroundColor: cardBg }}>
                <Text className='text-[16px] font-semibold uppercase px-[12px] py-[14px]' style={{ color: textSecondary }}>{formatTaskDate(choooseDate)}</Text>
                <FlatList
                  ...
                  renderItem={({ item }) => (
                    <View className={`mb-1`} style={{ backgroundColor: cardBg }}>
                      <Animated.View
                        layout={Layout.springify().damping(18).stiffness(180)}
                        entering={FadeInDown.duration(200)}
                        exiting={FadeOutUp.duration(150)}
                        className={`w-full box-content rounded-[10px] pl-[21px] py-4 h-[48px] ${item.subtasks?.length > 0 ? 'pr-0' : 'pr-4'} ${expandedTaskIds[item.task.id] && item.subtasks?.length > 0 ? '' : ''} flex-row items-center justify-between`}
                      ...
                      >
                        ...
                        >
                          <View>
                            <View>
                              <Text className={`text-[13px] ${item.subtasks?.length > 0 ? 'translate-x-[13px] ' : 'translate-x-2'}`} style={{ color: colorTheme }}>{item.task.time !== 'None' ? item.task.time : formatTaskDate(item.task.date)}</Text>
                            </View>
                            <View></View>
                          </View>
                        </TouchableOpacity>
                            ...
                            >
                              <TouchableWithoutFeedback onPress={() => {
                                ...
                                <View className='flex-row items-center justify-between w-full'>
                                  ...
                                  <View>
                                    <Text className={`text-[12.5px] ${subtask.status !== 'pending' ? 'text-gray-400 opacity-90' : subtask.time === 'None' && !isNotPastDate(subtask.date) ? 'text-red-500' : ''}`} style={{ color: subtask.status !== 'pending' ? '#9CA3AF' : subtask.time === 'None' && !isNotPastDate(subtask.date) ? '#EF4444' : colorTheme }}>{subtask.time !== 'None' ? subtask.time : formatTaskDate(subtask.date)}</Text>
                                  </View>
                                </View>
                          ...
                          <View className='flex-row items-center justify-between w-full'>
                            ...
                            >
                              <View className='flex-row items-center gap-x-3'>
                                ...
                                <View className='w-[72%]'>
                                  <Text className='text-[15.5px] line-through' style={{ color: '#9CA3AF' }} numberOfLines={1} ellipsizeMode="tail">{item.task.taskname}</Text>
                                </View>
                              </View>
                              <View>
                                <View>
                                  <Text className='text-[13px] opacity-90' style={{ color: '#9CA3AF' }}>{item.task.time !== 'None' ? item.task.time : formatTaskDate(item.task.date)}</Text>
                                </View>
                                <View></View>
                              </View>
                            </TouchableOpacity>
                            {item.subtasks?.length > 0 && (
                              ...
                              >
                                {expandedTaskIds[item.task.id]
                                  ? <AppIcon name="ChevronDown" color={isDark ? '#9CA3AF' : '#9ca3af'} size={15} />
                                  : <AppIcon name="chevronRight" color={isDark ? '#9CA3AF' : '#9ca3af'} size={15} />
                                }
                              </TouchableOpacity>
                            )}
                          </View>
                                ...
                                <TouchableWithoutFeedback onPress={() => {
                                  ...
                                  <View className='flex-row items-center justify-between w-full'>
                                    <View className='flex-row items-center gap-x-3'>
                                      ...
                                      <View className='w-[72%]'>
                                        <Text className={`text-[14.5px] ${subtask.status !== 'pending' ? 'line-through' : ''}`} style={{ color: '#9CA3AF' }} numberOfLines={1} ellipsizeMode="tail">{subtask.taskname}</Text>
                                      </View>
                                    </View>
                                    <View>
                                      <View>
                                        <Text className='text-[12.5px] opacity-90' style={{ color: '#9CA3AF' }}>{subtask.time !== 'None' ? subtask.time : formatTaskDate(subtask.date)}</Text>
                                      </View>
                                      <View></View>
                                    </View>
                                  </View>
            ...
            {completeTasks.length <= 0 && incompleteTasks.length <= 0
              ? <View className='flex-1 justify-center items-center pt-16'>
                ...
                <View className='mt-12'>
                  <Text className='text-center font-bold text-[15px] tracking-wide' style={{ color: textPrimary }}>You have a free day</Text>
                  <Text className='text-center font-light tracking-wide mt-2' style={{ color: textSecondary }}>Task it easy</Text>
                </View>
              </View>
              : ''
            }
